...
DATA_ROOT = "/mnt/DATA/Datasets/tum-rgbd/rosbags"

RESULT_ROOT = os.path.join(os.environ["SLAM_RESULT"], "dsol/tum-rgbd/gp")
DSOL_PATH = os.path.join(os.path.expanduser("~"), "catkin_ws")
# SeqNameList = [Path(x).stem for x in sorted(glob.glob(os.path.join(DATA_ROOT, "*.bag"))) if ".orig" not in x]
SeqNameList = ["rgbd_dataset_freiburg2_xyz"]
SeqFreq = 30.0
NumRepeating = 1
SleepTime = 5  # 10 # 25 # second
FeaturePool = [1000]  # [100, 150, 200] # , 800, 1000]
SpeedPool = [0.5]  # , 2.0, 3.0, 4.0, 5.0]  # , 2.0, 3.0, 4.0, 5.0] # x
EnableViewer = 1
LoggingLevel = 0
    ...
    for speed in SpeedPool:
        ...
        for iteration in range(NumRepeating):
            ...
            for sn, sname in enumerate(SeqNameList):
                ...
                # compose cmd
                cmd_slam = (
                    "roslaunch dsol dsol_odom_tum_rgbd.launch"
                    + " vis:="
                    + str(EnableViewer)
                    + " save:="
                    + file_traj
